...
uint8_t Si1132::readUV0()
{
	return read8(Si1132_REG_UVINDEX0);
}

uint8_t Si1132::readUV1()
{
...
uint8_t Si1132::read8(uint8_t reg)
{
	uint16_t val;
...
uint16_t Si1132::read16(uint8_t reg)
{
	uint16_t ret;
...
void Si1132::write8(uint8_t reg, uint8_t val)
{
	Wire.beginTransmission(_addr);
	Wire.write(reg);
	Wire.write(val);
...
uint8_t Si1132::readParam(uint8_t p)
{
...
uint8_t Si1132::writeParam(uint8_t p, uint8_t v)
{
	write8(Si1132_REG_COMMAND, Si1132_NOP); // clear the register
